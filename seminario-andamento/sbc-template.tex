...
Para que as informações adquiridas através do módulo de coleta possam ser utilizadas é necessário que estas cheguem até o processo do escalonador, em execução na máquina que possui o serviço Resource Manager. A escolha para a implementação desta comunicação no sentido escravo-mestre foi feita visando a compatibilidade com o Hadoop e a não intrusão nos processos de comunicação já definidos. Para isso optou-se por utilizar uma estrutura de HashTable distribuída (DHT) no \textit{znode}, na qual os escravos atualizam e o mestre pode ler as informações atualizadas.

%Na Figura \cite{fig:zk-usage} 
O funcionamento se dá da seguinte forma: todos os escravos possuem uma \textit{thread} chamada NodeStatusUpdater, esta \textit{thread} coleta dados sobre a disponibilidade de recursos do nó a cada 30 segundos (periodo parametrizável) e, se a quantidade de recursos disponíveis estiver diferente da última leitura, a DHT do \textit{znode} será atualizada. Concorrente a isto, o mestre possui uma \textit{thread watcher} que observa a DHT e caso esta seja atualizada, a \textit{thread watcher} será notificada e atualizará as informações no escalonador de acordo com a nova informação lida na DHT.
...
\textbf{Caso C:} repete as especificações do Caso B, porém possui a implementação da proposta na forma de coletores de contexto e comunicação entre nós. Este caso simula quando uma outra aplicação é lançada \textbf{antes} da ocorrência da coleta e transmissão de dados, ou seja, quando uma nova aplicação for submetida ao \textit{cluster}, este já estará com os dados atualizados. Em notação percentual, os recursos informados são de 50\% e os recursos disponíveis são de 50\%.
...
Os resultados dos experimentos estão resumidos na Tabela \ref{tab:resumo} e na Figura \ref{fig:gantts}. Na Tabela \ref{tab:resumo}, as colunas representam, respectivamente, da esquerda para direita: os casos explicados na seção anterior, o tempo total utilizado por todas as tarefas de Map, o tempo médio de execução das tarefas de Map, o desvio padrão do tempo médio de cada caso e o número de tarefas especulativas lançadas.
...
\hline
Caso & Tempo total({\it{s}}) & Tempo médio ({\it{s}}) & Desvio padrão ({\it{s}}) & Tarefas especulativas\\
\hline
A & 149 & 39.47 & 15.73 & 2\\
